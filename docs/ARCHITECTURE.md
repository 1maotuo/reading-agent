...
- 书籍类型 Skill：通用、哲学、历史、人文社科、科学科普、实用方法、小说叙事；导入时按BookProfile动态选择，低置信度回退通用类型。
...
书籍类型在导入阶段根据元数据、目录、前言和样本判断；模型可用时做结构化语义分类，不可用或置信度不足时显式降级为通用/确定性分类，并允许用户覆盖。Agent Core V1已加载版本化七类结构Skill和受限用户表达偏好，Skill版本与分类来源进入Trace；Skill只影响阅读方法和表达方式，不改变事实、权限和数据边界。

...
当前版本是 `0.0.6-stage05-conversation-preview`。这是开发预览，不是生产版本；Gate 05 与 Gate 06 尚未正式通过。
...
- 本地预览保存书籍结构、进度、划线、证据和问答状态；登录会话不会作为生产账号系统持久化。
- 本地检索是受书籍范围约束的开发实现，不代表生产 pgvector 的性能或多端同步能力。
- 真实模型质量、生产认证、独立 Worker、完整跨设备同步、云对象存储、语音和部署仍未完成。

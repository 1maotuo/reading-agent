...
- 总预算默认约 4200 个估算 token；允许范围 1200–8000。
- 当前问题最多约 600 token。
...
`ReadingMemoryStore` 在记忆开关开启时，把每个完成的书籍回答压成：
...
### 3. 结构化书籍学习记忆：🟠 数据结构较完整，自动闭环只接了一小段
...
- 书籍问题被判定为用户明确困惑时，写入一个 `UNCONFIRMED` 学习事件；
- 同时写入一个 `CANDIDATE` 困惑信号；
- 下次相关问题可按词面相关度召回这类事件；
- 非书籍问题、记忆关闭、用户未明确困惑、失败或取消的回答不会写入。

尚未接通的关键闭环：

- 导入书籍时没有生成稳定的全书 `concept_id`；
- 自动写回的学习事件目前 `concept_id=None`；
- 没有把用户后续的“对/不对、现在懂了、我能复述”等反馈自动关联到旧事件；
- 候选信号没有在运行时经过确认并推动 `ConceptState` 迁移；
- 概念状态与 Profile 虽可计算，但真实使用中通常缺少可用概念状态；
- 没有独立的全局长期用户画像、跨书偏好学习和遗忘策略。

所以不能把这些类的存在理解成“深度记忆已经完成”。更准确的说法是：**结构化记忆领域模型已经搭好，召回与困惑事件写回已接入，理解状态的学习闭环尚未完成。**
